...
#         # u = C (M - hL)^{-1} M d
#         #       ^--A--^
#         self.A = massmatrix_tets(self.vertices,self.elements).get() - wave_h*igl.cotmatrix(self.vertices,self.elements)
#         self.M = massmatrix_tets(self.vertices,self.elements).get()
...
class fracture_modes:
    ...
    def impact_precomputation(self, v_fine=None, f_fine=None, wave_h=1 / 30, upper_envelope=False,
        ...
        tet_tet_distances_rep = np.abs(
            self.modes[ind2dim(self.tet_neighbors[:, 0]), :] - self.modes[ind2dim(self.tet_neighbors[:, 1]),
                                                               :])  # This is a dim x num_neighbor_pairs by num_modes matrix
        ...
        def solve_mode(k):
            # 处理单个 mode k 的函数
            # 这里使用局部变量 blockdiag_mat, piece_to_tet_matrix, modes, labels，需要在函数前已定义为可访问
            x_mode = lsqr(kron(blockdiag_mat, self.piece_to_tet_matrix), self.modes[:, k])[0]
            x_label = np.rint(lsqr(self.piece_to_tet_matrix, self.labels[:, k])[0]).astype(int)
            return k, x_mode, x_label

        # 使用线程池并行计算每个 mode
        num_modes = self.modes.shape[1]
        results = []
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(solve_mode, k) for k in range(num_modes)]
            for fut in futures:
                k, x_mode, x_label = fut.result()
                self.piece_modes[:, k] = x_mode
                self.piece_labels[:, k] = x_label
        ...
        # u = C (M - hL)^{-1} M d
        #       ^--A--^
        self.A = massmatrix_tets(self.vertices, self.elements).get() - wave_h * igl.cotmatrix(self.vertices,
                                                                                              self.elements)
        self.M = massmatrix_tets(self.vertices, self.elements).get()
        # (C blurs per-unexploded-vertex values into tets)
        self.C = 0.25 * (self.tet_to_vertex_matrix.T @ self.unexploded_to_exploded_matrix)
        ...
        if (v_fine is not None):
            ...
            for k in range(self.modes.shape[1]):
                self.fine_labels[:, k] = self.piece_to_fine_vertices_matrix @ \
                                         lsqr(self.piece_to_tet_matrix, self.labels[:, k])[
                                             0]  # We don't really need this lsqr (self.labels is constant per piece), but this is not a bottleneck.
        else:
            self.fine_vertices = None
            self.fine_triangles = None
        ...
        self.t_impact_pre = round(toc(silence=True), 5)
        if self.verbose:
            print("Impact precomputation: ", self.t_impact_pre, " seconds. Will produce a maximum of",
                  self.precomputed_num_pieces, "pieces.")
        # This is a boolean that we'll check before projecting an impact
        self.impact_precomputed = True
    ...
    def impact_projection(self, contact_point=None, threshold=0.02, wave=True, direction=np.array([1]), impact=None,
        ...
        # Calculate the difference in displacements between neighboring pieces (we use the piece adjancency we precomputed)
        import cupy as cp
        piece_neighbors_gpu = cp.asarray(self.piece_neighbors)
        projected_impact_gpu = cp.asarray(self.projected_impact)
        projected_impact_reshaped_gpu = projected_impact_gpu.reshape(-1, dim, order='F')
        diff_gpu = projected_impact_reshaped_gpu[piece_neighbors_gpu[:, 0], :] - projected_impact_reshaped_gpu[
                                                                                 piece_neighbors_gpu[:, 1], :]
        piece_distances_gpu = cp.linalg.norm(diff_gpu, axis=1)
        piece_distances = cp.asnumpy(piece_distances_gpu)  # If connected_components requires CPU data later, transfer it back to the CPU

        # Subsequent steps, such as connected_components, are still performed on the CPU.
        piece_neighbors_after_impact = self.piece_neighbors[piece_distances < threshold, :]
        # Build an impact-dependent piece adjancency graph
        piece_piece_adjacency_after_impact = csr_matrix((np.ones(piece_neighbors_after_impact.shape[0]), (
        piece_neighbors_after_impact[:, 0], piece_neighbors_after_impact[:, 1])), shape=(
        self.precomputed_num_pieces, self.precomputed_num_pieces), dtype=int)
        ...
        for j in range(self.modes.shape[1]):
            ...
            for i in range(np.max(self.fine_labels[:, j]) + 1):  #
                ...
                if (self.fine_vertices is not None):
                    tri_labels = self.fine_labels[self.fine_triangles[:, 0], j]
                    if np.any(tri_labels == i):
                        vi, fi = igl.remove_unreferenced(self.fine_vertices, self.fine_triangles[tri_labels == i, :])[
                                 :2]
            ...
            if (filename is not None):
                if (not pieces):
                    igl.write_obj(filename + "_mode_" + str(j) + ".obj", self.mesh_to_write_vertices,
